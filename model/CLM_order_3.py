...
class CrossLabelMask_Model_order_3(nn.Module):
    def __init__(self,
                plm:PreTrainedModel,
    ...
    def forward(self,batch):
        ...
        cpu_outputs_at_mask = outputs_at_mask[0+15] + outputs_at_mask[1+15] + outputs_at_mask[2+15] + outputs_at_mask[3+15] + outputs_at_mask[4+15] + outputs_at_mask[5+15] + outputs_at_mask[0] + outputs_at_mask[0+5]+outputs_at_mask[1+5]+outputs_at_mask[2+5]+outputs_at_mask[3+5]
        cpu_label_words_logits = self.cpu_promptVerbalizer.process_outputs(cpu_outputs_at_mask, batch=batch)
        
        graphic_outputs_at_mask = outputs_at_mask[0+15] + outputs_at_mask[1+15] + outputs_at_mask[2+15] + outputs_at_mask[6+15] + outputs_at_mask[7+15] + outputs_at_mask[8+15] + outputs_at_mask[1] + outputs_at_mask[0+5]+outputs_at_mask[4+5]+outputs_at_mask[5+5]+outputs_at_mask[6+5]
        graphic_label_words_logits = self.graphic_promptVerbalizer.process_outputs(graphic_outputs_at_mask, batch=batch)
        
        hardisk_outputs_at_mask = outputs_at_mask[0+15] + outputs_at_mask[3+15] + outputs_at_mask[4+15] + outputs_at_mask[6+15] + outputs_at_mask[7+15] + outputs_at_mask[9+15] + outputs_at_mask[2] + outputs_at_mask[1+5]+outputs_at_mask[4+5]+outputs_at_mask[7+5]+outputs_at_mask[8+5]
        hardisk_label_words_logits = self.hardisk_promptVerbalizer.process_outputs(hardisk_outputs_at_mask, batch=batch)
        
        ram_outputs_at_mask = outputs_at_mask[1+15] + outputs_at_mask[3+15] + outputs_at_mask[5+15] + outputs_at_mask[6+15] + outputs_at_mask[8+15] + outputs_at_mask[9+15] + outputs_at_mask[3] + outputs_at_mask[2+5]+outputs_at_mask[5+5]+outputs_at_mask[7+5]+outputs_at_mask[9+5]
        ram_label_words_logits = self.ram_promptVerbalizer.process_outputs(ram_outputs_at_mask, batch=batch)
        
        screen_outputs_at_mask = outputs_at_mask[2+15] + outputs_at_mask[4+15] + outputs_at_mask[5+15] + outputs_at_mask[7+15] + outputs_at_mask[8+15] + outputs_at_mask[9+15] + outputs_at_mask[4] + outputs_at_mask[3+5]+outputs_at_mask[6+5]+outputs_at_mask[8+5]+outputs_at_mask[9+5]
...
if __name__ == '__main__':
    ...
    epoch=30
    template='{"soft": "unused unused unused unused"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"placeholder":"text_a"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"soft": "unused unused"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"mask"} {"soft": "unused unused"}'
    
    
    model=CrossLabelMask_Model_order_3(plm,
                                   tokenizer,
                                   WrapperClass,
                                   dataset,
                                   need_dataset,
    ...
    for i in range(model.epoch):
        ...
        if(i==9 or i==14 or i==19 or i==24 or i==29):
            ...
            PATH = "./../autodl-tmp/checkpoint/CMFv3_CEL/raw_data_run2/CMFv3_CEL_Epoch_"+str(i+1)+".pt"
            torch.save({
                    'epoch': i+1,
    ...
    for i in range(20):
        ...
        if(i==4 or i==9 or i==14 or i==19):
            ...
            save_path="./result/CMFv3_CEL/raw_data_run2/CMFv3_CEL_all_epoch_"+str(i+1)+"_test_res.csv"
            n=len(cpu_labels)
            record=[]
            for j in range(0,n):
